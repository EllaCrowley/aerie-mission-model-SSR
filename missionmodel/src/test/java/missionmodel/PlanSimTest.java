...
public class PlanSimTest {
  ...
  final void testSchedulePeriodicDataCollections() {
    ...
    plan.create(Utils.getGroundContactActivityName(), new DirectiveStart.Absolute(Duration.hours(2)), Map.of());
    plan.create(Utils.getGroundContactActivityName(), new DirectiveStart.Absolute(Duration.hours(12)), Map.of());

    // Simulate Plan
    SimulationResults simResults = plan.simulate();

    // Perform assertions
    Numbers<Number> modeProfile = simResults.resource(PowerModel.FLIGHT_COMPUTER_DRAIN_RESOURCE_NAME, Numbers.deserializer());
    assertEquals(PowerModel.FLIGHT_COMPUTER_DRAIN_RATE, Double.valueOf(String.valueOf(modeProfile.sample(Duration.hours(0.5)))));

    Numbers<Number> rateProfile = simResults.resource(PowerModel.SOLAR_ARRAY_CHARGE_RESOURCE_NAME, Numbers.deserializer());
